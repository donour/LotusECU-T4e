...
class Patcher:
    ...
    def check_blank(self, addr, size, blank=0xFF):
        ...
        for i in range(addr, addr + size):
            if self.data[i] != blank:
                raise Exception("Not blank!")
...
class Patcher_Prog(Patcher):
    ...
    def read_segments(self):
        ...
        if i == -1:
            raise Exception("First segment not found!")
        ...
        while self.data[i : i + 4] == b"\x00\x00\x00\x00":
            i += 4
        ...
        while self.data[i : i + 4] == b"\xFF\xFF\xFF\xFF":
            i += 4
        self.blank_limit = i
...
class Patcher_T4eCalibration(Patcher):
    def __init__(self, cal_file, free_cal=None):
        ...
        if free_cal == None:
            self.free_cal = self.get_free_space()
        else:
            self.free_cal = free_cal
    ...
    def __init__(self, file):
        ...
        with open(file, "r") as f:
            for line in f.readlines():
                m = r_sym.match(line)
                if m:
                    self.syms[m.group(2)] = int(m.group(1), 16)
                m = r_seg.match(line)
                if m:
                    self.segs[m.group(1)] = (int(m.group(2), 16), int(m.group(3), 16))
...
def build_stage15():
    ...
    p.search_and_replace(
        PPC32.ppc_ori(
            4, 4, 0x1FDC
        ),  # This value is also hardcoded in canstrap-black.bin
...
def build_accusump():
    ...
    os.system(
        "make -C t4e/accusump CAL=0x{:X} ROM=0x{:X} RAM=0x{:X}".format(
            c.get_free_cal(),
            acis,  # p.get_free_rom(),
            p.get_free_ram(),
        )
...
def build_obdoil():
    ...
    os.system(
        "make -C t4e/obdoil CAL=0x{:X} ROM=0x{:X} RAM=0x{:X} SYM={:s}".format(
            c.get_free_cal(), p.get_free_rom(), p.get_free_ram(), "../black91.sym"
        )
    )
...
def build_accusump2():
    ...
    p = Patcher_T4eProg("../dump/t4e-black/A129E0002/prog.bin")
    os.system(
        "make -C t4e/accusump2 CAL=0x{:X} ROM=0x{:X} RAM=0x{:X} SYM={:s}".format(
            c.get_free_cal(), p.get_free_rom(), p.get_free_ram(), "../black91.sym"
        )
    )
    s = SYMMap("t4e/black91.sym")
...
def build_flexfuel():
    ...
    p = Patcher_T4eProg("../dump/t4e-black/A129E0002/prog.bin")
    os.system(
        "make -C t4e/flexfuel CAL=0x{:X} ROM=0x{:X} RAM=0x{:X} SYM={:s}".format(
            c.get_free_cal(), p.get_free_rom(), p.get_free_ram(), "../black91.sym"
        )
    )
    s = SYMMap("t4e/black91.sym")
    ...
    for i in range(0, 16):
        c.data[addr_dst + i] = c.data[addr_src + i]

    # Copy ignition (high cam) table for ethanol.
    addr_src = s.get_sym_addr("CAL_ign_advance_high_cam_base") - s.get_sym_addr(
        "CAL_base"
    )
    ...
    addr_src = s.get_sym_addr("CAL_ign_advance_low_cam_base") - s.get_sym_addr(
        "CAL_base"
    )
...
def build_wideband():
    ...
    p = Patcher_T4eProg("../dump/t4e-black/A129E0002/prog.bin")
    os.system(
        "make -C t4e/wideband CAL=0x{:X} ROM=0x{:X} RAM=0x{:X} SYM={:s}".format(
            c.get_free_cal(), p.get_free_rom(), p.get_free_ram(), "../black91.sym"
        )
    )
    s = SYMMap("t4e/black91.sym")
    ...
    p.check_and_replace(0x0339B0, PPC32.ppc_li(4, 0), PPC32.ppc_li(4, 1))
...
def build_t6_flexfuel():
    ...
    os.system(
        "make -C t6/flexfuel CAL=0x{:X} ROM=0x{:X} RAM=0x{:X} SYM={:s}".format(
            c.get_free_cal(), p.get_free_rom(), p.get_free_ram(), "../T6-V000S.sym"
        )
    )
    ...
    p.check_and_replace(0x4285C, PPC32.ppc_li(0, 0x0100), PPC32.ppc_li(0, 0x0500))
    ...
    p.check_and_replace(0x43050, PPC32.ppc_ble(-0x280), PPC32.ppc_ble(0x1C))
