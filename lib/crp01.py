...
class CRP01_hdr_ecu_k4(BinData):
    ...
    def parse(self, data):
        for i in range(0, 7):
            if data[i] == 0:
                self.erase_sector[i] = False
            elif data[i] == 1:
                self.erase_sector[i] = True
            else:
...
"""
        return fmt.format(*["Yes" if x else "No" for x in self.erase_sector[3:]])
...
class CRP01_hdr_ecu_t4e(BinData):
    ...
    def set_erase_by_addr(self, addr):
        sector = addr // 0x10000
        if sector == 0:
            self.erase_sector[0] = True
        elif sector == 1:
            self.erase_sector[2] = True
        elif sector < 8:
            self.erase_sector[1] = True
...
"""
        return fmt.format(*["Yes" if x else "No" for x in self.erase_sector])
...
class CRP01_subpackets(BinData):
    ...
    def delete(self, offset, size):
        limit = offset + size
        self.subpackets = [
            x for x in self.subpackets if not (offset <= x[0] and x[0] < limit)
        ]
    ...
    def import_srec(self, file):
        ...
        for line in data_srec.split("\n"):
            if len(line) < 2 or line[0] != "S":
                continue
            srec_bin = bytearray(
                [int(line[i : i + 2], 16) for i in range(2, len(line), 2)]
            )
            length = srec_bin[0]
            ...
            if len(data) % 2 != 0:
                raise Exception(
                    "S-Record uneven length is incompatible with encryption!"
                )
    ...
    def export_bin(self, file, offset, size):
        ...
        for s in self.subpackets:
            if not (offset <= s[0] and s[0] < limit):
                continue
            x = s[0] - offset
            buf[x : x + len(s[1])] = s[1]
...
class CRP01(BinData):
    ...
    def __init__(self, i=0):
        ...
        if self.is_encrypted:
            ...
            self.data = CRP01_data_ecu(CRP01.variants[i][1], CRP01.variants[i][2])
        self.file_data = b""
...
CRP01 K-Line File:
	...
	Description : {:s}
"""
        return fmt.format(self.desc) + str(self.data)
