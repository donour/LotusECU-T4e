...
class CRP08_xtea:
    ...
    def encrypt(self, v0, v1):
        ...
        for i in range(0, self.rounds):
            v0 = (
                v0 + (((v1 << 4 ^ v1 >> 5) + v1) ^ (xsum + self.key[xsum & 3]))
            ) & self.mask
            xsum = (xsum + self.delta) & self.mask
            v1 = (
                v1 + (((v0 << 4 ^ v0 >> 5) + v0) ^ (xsum + self.key[(xsum >> 11) & 3]))
            ) & self.mask
        return v0, v1
...
class CRP08_chunk_toc(BinData):
    ...
    def compose(self, data):
        ...
        for i in range(0, len(self.toc_values)):
            ...
            for j in range(0, len(self.toc_values[i])):
                x = offset + 4 + (self.ENS * j)
                data[x : x + self.ENS] = bytes(
                    self.toc_values[i][j].ljust(self.ENS), CHARSET
                )
            offset += size
...
CRP08 TOC Chunk:
	...
	Desc. : {:s}
"""
        return fmt.format(" ".join(self.toc_values[0]), " ".join(self.toc_values[1]))
...
class CRP08_chunk_can(BinData):
    ...
    def parse(self, data):
        ...
        if self.is_encrypted:
            ...
            self.data.parse(data[64:])

    def get_size(self):
        if self.is_encrypted:
            return 64 + len(self.enc_data)
        else:
            return 64 + self.data.get_size()
...
class CRP08(BinData):
    variants = [
        ...
        [
            "T6 Caterham",
            CRP08_xtea.T6_CATERHAM_KEY,
            "CATERHAM T6",
            0x5,
            0x4,
            False,
            "CATERHAM_ECU_DURATEC",
        ],
        [
            "CT1 Caterham",
            CRP08_xtea.T6_CATERHAM_KEY,
    ...
    def parse(self, data, key):
        ...
        for i in range(0, len(self.chunks)):
            ...
            size = int.from_bytes(data[x + 4 : x + 8], BO_LE)
            if i == 0:
                chunk = CRP08_chunk_toc()
            else:
                chunk = CRP08_chunk_can(key)
