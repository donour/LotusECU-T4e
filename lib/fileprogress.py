...
class FileProgress(Progress):
    def __aligned(self, size, chunk_size, chunk_align):
        if size % chunk_size != 0 and chunk_align:
            raise FileProgressException(
                f"File size is not a multiple of {chunk_size:d}"
            )
    ...
    def verify(
        self, address, filename, read_fnct, chunk_size, chunk_align, offset=0, size=None
    ):
        if not size:
            size = os.path.getsize(filename) - offset
        self.log(
            f"Verify {size:d} bytes @ 0x{address:08X} from {filename} +0x{offset:X}"
        )
        self.__aligned(size, chunk_size, chunk_align)
    ...
    def verify_blank(self, address, size, read_fnct, chunk_size, chunk_align):
        ...
        while size > 0:
            ...
            for byte in chunk:
                if byte != 0xFF:
                    raise FileProgressException(
                        f"Verify Blank failed! @ 0x{address:08X}"
                    )
            self.progress(chunk_size)
            address += chunk_size
            size -= chunk_size
    ...
    def upload(
        ...
        if not size:
            size = os.path.getsize(filename) - offset
        self.log(
            f"Upload {size:d} bytes @ 0x{address:08X} from {filename} +0x{offset:X}"
        )
        self.__aligned(size, chunk_size, chunk_align)
        with open(filename, "rb") as f:
            f.seek(offset)
    ...
    def watch(
        self, address, filename, write_fnct, offset=0, size=None, ui_cb=lambda: None
    ):
        if not size:
            size = os.path.getsize(filename) - offset
        self.log(
            f"Watch {size:d} bytes @ 0x{address:08X} from {filename} +0x{offset:X}"
        )
        ...
        while True:
            ...
            if cache_ts != ts:
                ...
                while i < min(len(cache_data), len(data)):
                    j = i
                    while cache_data[j] != data[j] and (j - i) < 255:
                        j += 1
                    if j > i:
