...
if __name__ == "__main__":
    ...
    crc.update(data)
    if crc.get() == 0x02:
        print("CRC-8/CDMA2000 is OK")
    crc.update_reverse(data)
    if crc.get() == 0xFF:
        print("Reverse CRC-8/CDMA2000 is OK")

    # Test CRC-8/DARC
    crc = CRC8Reflect(0x39, initvalue=0x00)
    crc.update(data)
    if crc.get() == 0xA1:
        print("CRC-8/DARC is OK")
    crc.update_reverse(data)
    if crc.get() == 0x00:
        print("Reverse CRC-8/DARC is OK")

    # Test CRC-16/CCITT-FALSE
    crc = CRC16Normal(0x1021, initvalue=0xFFFF)
    crc.update(data)
    if crc.get() == 0x8FDD:
        print("CRC-16/CCITT-FALSE is OK")
    crc.update_reverse(data)
    if crc.get() == 0xFFFF:
        print("Reverse CRC-16/CCITT-FALSE is OK")

    # Test CRC-16/KERMIT
    crc = CRC16Reflect(0x1021, initvalue=0x0000)
    crc.update(data)
    if crc.get() == 0xC459:
        print("CRC-16/KERMIT is OK")
    crc.update_reverse(data)
    if crc.get() == 0x0000:
        print("Reverse CRC-16/KERMIT is OK")

    # Test CRC-32/MPEG-2
    crc = CRC32Normal(0x04C11DB7, initvalue=0xFFFFFFFF)
    crc.update(data)
    if crc.get() == 0xBA62119E:
        print("CRC-32/MPEG-2 is OK")
    crc.update_reverse(data)
    if crc.get() == 0xFFFFFFFF:
        print("Reverse CRC-32/MPEG-2 is OK")

    # Test CRC-32C
    crc = CRC32Reflect(0x1EDC6F41, initvalue=0xFFFFFFFF)
    crc.update(data)
    if crc.get() ^ 0xFFFFFFFF == 0x22620404:
        print("CRC-32C is OK")
    crc.update_reverse(data)
    if crc.get() == 0xFFFFFFFF:
        print("Reverse CRC-32C is OK")
