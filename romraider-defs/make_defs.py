...
def parse_CALentry(s):
    ...
    m = pattern.match(s)
    if not m:
        raise Exception("Error parsing CAL/LEA: " + s)
...
def parse_var(s):
    ...
    m = pattern.match(s)
    if not m:
        raise Exception("Error parsing var: " + s)
...
def xml_add_signature(rom, s):
    ...
    d = s.split("#")
    if len(d) < 11:
        raise Exception("XXX_base has not enough info!")
...
OBD2MONITORS = [
    (
        "Components, Fuel System, Misfire, O2 Sensor Heater, O2 Sensor, Evaporative System, Catalyst",
        "07 65",
    ),
    (
        "Components, Fuel System, Misfire, O2 Sensor Heater, O2 Sensor, Catalyst",
        "07 61",
    ),
...
USE_TPMS = [("No TPMS", "00"), ("Use TPMS module", "01")]
...
def do(symbols, units, xaxis, calrom, decram):
    ...
    for s in symbols:
        ...
        if data[3] == "u8_obd2level":
            ...
            xml_add_switch(rom, data, OBD2MONITORS)
        elif s == "CAL_ecu_unlock_magic":
            xml_add_switch(rom, data, UNLOCK_MAGIC)
        elif s == "CAL_misc_use_tmap":
            xml_add_switch(rom, data, USE_TMAP)
        elif s == "CAL_misc_use_tpms":
            xml_add_switch(rom, data, USE_TPMS)
        elif s == "CAL_tc_mode":
            xml_add_switch(rom, data, TC_MODE)
            ...
            if s.startswith("CAL_misc_shift_lights_before_rev_limit"):
                ...
                if dim == 1:
                    if s in xaxis:
                        xml_add_2D_static(rom, data, xaxis[s][0 : data[4]])
                    elif data[4] > 1:
                        ...
                        xml_add_2D_static(
                            rom,
                            data,
                            [str(i << (8 - shift)) for i in range(0, data[4])],
                            xname,
                            units[xunit],
                        )
                    else:
                        xml_add_2D_static(rom, data)
                elif dim == 2:
                    xml_add_2D(rom, data, d_xaxis)
                elif dim == 3:
                    xml_add_3D(rom, data, d_xaxis, d_yaxis)
                dim = 1
        else:
...
def find_symbols_files():
    ...
    for filename in os.listdir("."):
        ...
        if m:
            result.append((filename, m.group(1) + "_defs.xml"))
    return result
