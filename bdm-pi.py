...
class BDM_PI:
    ...
    def io_bytes(self, bytes_in):
        ...
        for byte_in in bytes_in:
            bytes_out.append(self.io_byte(byte_in))
        return bytes_out
    ...
    def execute(self, address, msr=0x000003002):
        ...
        self.io_bytes(b"\x05" + address.to_bytes(4, "big"))
        self.io_bytes(
            b"\x04" + PPC32.ppc_mtspr(30, 26)
        )  # Set the program counter by setting SRR0
        self.io_bytes(b"\x04" + PPC32.ppc_mfspr(31, 630))
        self.io_bytes(b"\x05" + msr.to_bytes(4, "big"))
        self.io_bytes(
            b"\x04" + PPC32.ppc_mtspr(31, 27)
        )  # Set SRR1 to desired MSR register
        self.io_bytes(
            b"\x04" + PPC32.ppc_mfspr(30, 148)
        )  # Read the ECR register to clear out any exceptions
