...
class Flasher:
    ...
    def recv(self, cmd, length=0, timeout=1.0):
        ...
        if msg == None:
            raise FlasherException("No answer!")
    ...
    def program(self, block_mask, address, filename, offset=0, size=None):
        try:
            ...
            self.fp.upload(
                address, filename, self.program_block_word, 4, True, offset, size
            )
        finally:
    ...
    def canstrap(self, timeout=60, ui_cb=lambda: None):
        ...
        for _ in range(0, int(timeout / 0.5)):
            ...
            if msg != None:
                break
        if msg == None:
            raise FlasherException("Time out!")
    ...
    def test(self, freeram_address):
        ...
        if msg == None:
            raise FlasherException("Test failed!")
...
if __name__ == "__main__":
    ...
    if ecu_op == "v":
        print("Verify ECU")
        for i in ecu_blocks:
            fl.verify(Flasher.blocks[i][2], ecu_dir + "/" + Flasher.blocks[i][4])

    if ecu_op == "vb":
        print("Verify Blank ECU")
        for i in ecu_blocks:
            fl.verify_blank(Flasher.blocks[i][2], Flasher.blocks[i][3])
