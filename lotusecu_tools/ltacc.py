...
class LiveTuningAccess:
    ...
    def read_memory(self, address, size):
        if size == 4:
            ...
            if msg == None:
                raise ECUException("ECU Read Word failed!")
            if msg.dlc != 4:
            ...
            msg = self.bus.recv(timeout=1.0)
            if msg == None:
                raise ECUException("ECU Read Half failed!")
            if msg.dlc != 2:
                raise ECUException("Unexpected answer!")
            data = msg.data
        elif size == 1:
            ...
            msg = self.bus.recv(timeout=1.0)
            if msg == None:
                raise ECUException("ECU Read Byte failed!")
            if msg.dlc != 1:
                raise ECUException("Unexpected answer!")
            data = msg.data
        elif size < 256:
            ...
            while size > 0:
                ...
                msg = self.bus.recv(timeout=1.0)
                if msg == None:
                    raise ECUException("ECU Read Buffer failed!")
                if msg.dlc != chunk_size:
                    raise ECUException("Unexpected answer!")
                data += msg.data
                size -= chunk_size
        else:
    ...
    def read_ptrmap(self):
        msg = can.Message(is_extended_id=False, arbitration_id=0x53, data=bytes(4))
        self.bus.send(msg)
        msg = self.bus.recv(timeout=1.0)
        if msg == None:
            raise ECUException("ECU Read Map failed!")
        if msg.dlc != 4:
            raise ECUException("Unexpected answer!")
        ...
        while True:
            ...
            if entry == b"\x00\x00\x00\x00\x00\x00":
                break
            ptrmap.append(
                (
