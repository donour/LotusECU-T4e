...
class LiveTuningAccess_win(tk.Toplevel):
    def __init__(self, config, parent=None):
        ...
        self.combo_zones = ttk.Combobox(
            btn_frame, state="readonly", values=[z[0] for z in LiveTuningAccess.zones]
        )
        ...
        self.button_ifp = tk.Button(
            lta_frame, text="Inject T4e Custom Flasher Program", command=self.inject
        )
        self.button_ifp.pack(fill=tk.X)

        self.button_pmt = tk.Button(
            lta_frame, text="The poor man's live-tuning", command=self.watch
        )
...
""",
        ...
        copy = answer == "yes"
        answer = tk.messagebox.askquestion(
            parent=self, title=title, message="Should writing be verified?"
        )
        verify = answer == "yes"
        answer = filedialog.askopenfilename(
            parent=self,
