...
class Flasher_win(tk.Toplevel):
    def __init__(self, prefs, parent=None):
        ...
        self.button_b = tk.Button(
            fl_frame, text="Bootstrap from Stage 1.5 (60 sec.)", command=self.bootstrap
        )
        ...
        self.combo_blocks = ttk.Combobox(
            btn_frame, state="readonly", values=[b[0] for b in Flasher.blocks]
        )
    ...
    def reset(self, fl):
        answer = tk.messagebox.askquestion(
            parent=self, title="Be careful!", message="Do you really want to reset?"
        )
        if answer != "yes":
            return
        fl.branch(0x100)
