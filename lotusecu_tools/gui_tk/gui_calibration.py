...
class CAL_editor_win(tk.Toplevel):
    def __init__(self, prefs, parent=None):
        ...
        menu.add_command(
            label="Modify generic VIN to match CRC", command=self.match_crc2
        )
        menu.add_separator()
    ...
    def search(self):
        ...
        if answer:
            ...
            if offset == None:
                msg = "CRC cmplwi not found!"
            else:
                msg = f"CRC cmplwi offset: 0x{offset:06X}"
            messagebox.showinfo(master=self, title="T4e CRC cmplw offset!", message=msg)
    ...
    def match_crc(self):
        ...
        crc = simpledialog.askstring(
            "CRC", "Target CRC", parent=self, initialvalue=hex(self.cal.compute_crc())
        )
        self.grab_set()
        if crc == None:
            return
    ...
    def match_crc2(self):
        crc = simpledialog.askstring(
            "CRC", "Target CRC", parent=self, initialvalue=hex(self.cal.get_crc())
        )
        self.grab_set()
        if crc == None:
            return
